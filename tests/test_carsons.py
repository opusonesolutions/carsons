...
def CN_line_z_primitive():
    ...
    return array(
        [
            ...
            [0.0 + 0.0j, 0.0 + 0.0j,             0.0 + 0.0j,                0.0 + 0.0j],
            [0.0 + 0.0j, 0.0 + 0.0j, 7.5515e-04 + 1.006e-3j,  5.9217e-05 + 0.00047649j],
            [0.0 + 0.0j, 0.0 + 0.0j, 5.921e-05 + 0.00047649j, 7.5515e-04 + 0.00100631j],
        ]
    )
    # fmt: on
...
def z_abc_kersting_4_1():
    ...
    return array(
        [
            ...
            [0.1560 + 0.5017j, 0.4666 + 1.0 * 1.0482j, 0.1580 + 0.4236j],
...
@pytest.mark.parametrize(
    "line,z_primitive_expected", [(ABCN_balanced_line(), ABCN_balanced_z_primitive())]
)
...
@pytest.mark.parametrize(
    "z_abc,z_012_expected", [(z_abc_kersting_4_1(), z_012_kersting_4_1())]
)
