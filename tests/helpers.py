...
class ConcentricLineModel:
    def __init__(self, conductors):
        ...
        for ph, val in conductors.items():
            if "N" in ph:
                self._neutral_strand_gmr[ph] = val["neutral_strand_gmr"]
                self._neutral_strand_resistance[ph] = val["neutral_strand_resistance"]
                self._neutral_strand_diameter[ph] = val["neutral_strand_diameter"]
                self._diameter_over_neutral[ph] = val["diameter_over_neutral"]
                self._neutral_strand_count[ph] = val["neutral_strand_count"]
            else:
                self._resistance[ph] = val["resistance"]
                self._geometric_mean_radius[ph] = val["gmr"]
                self._wire_positions[ph] = val["wire_positions"]
            self._phases = sorted(list(conductors.keys()))
